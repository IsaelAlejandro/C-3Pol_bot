...
In the **Stokes Parameters Estimation** folder, execute the `Par_C3Pol.mat` script. This script requires the file `I_Cf1.mat`, which contains the synthetic data to be analyzed.

The folder must also contain the files `I0Lm.mat`, `I0Rm.mat`, `MBaseb.mat`, and `PinvBaseb.mat`, which are generated by the `SGM_bases_computation.m` script.

The `Par_C3Pol.mat` script generates 12 `.mat` files containing the estimated polarization and location parameters of the elliptical dipoles, including (\xi), (\eta), the degree of polarization (P), etc.
